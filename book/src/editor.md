...
| `gutters` | Gutters to display: Available are `diagnostics` and `diff` and `line-numbers` and `spacer` and `cells`, note that `diagnostics` also includes other features like breakpoints and a `◆` marker on closed folds, 1-width padding will be inserted if gutters is non-empty | `["diagnostics", "spacer", "line-numbers", "spacer", "diff", "cells"]` |
...
| Key      | Description                    | Default                                                       |
| ---      | ---                            | ---                                                           |
| `layout` | A vector of gutters to display | `["diagnostics", "spacer", "line-numbers", "spacer", "diff", "cells"]` |
...
#### `[editor.gutters.cells]` Section

The `cells` gutter displays a colored bar marking the extent of every notebook
cell in a percent-format buffer (`# %%` delimiters, including converted
`.ipynb` files), colored by cell type. It collapses to zero width in buffers
without cell markers. Colors are controlled by the theme attributes
`ui.gutter.jupyter.cell.code` and `ui.gutter.jupyter.cell.markdown` (falling
back to the `ui.virtual.jupyter.cell.*` delimiter styles).
